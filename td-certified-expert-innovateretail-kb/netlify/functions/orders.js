...
const s = v => (v == null ? "" : String(v));   // ensure strings, no nulls
...
exports.handler = async (event) => {
  ...
  // Normalize "+" to avoid space decoding issues
  ...
  if (results.length === 0)
    return json(404, { error: 'NotFound', message: 'No matching orders', count: 0, orders: [] });
